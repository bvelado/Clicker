...
public static class Pools {
    ...
    public static Pool uI {
        get {
            if (_uI == null) {
                _uI = new Pool(UIComponentIds.TotalComponents, 0, new PoolMetaData("UI Pool", UIComponentIds.componentNames, UIComponentIds.componentTypes));
                #if (!ENTITAS_DISABLE_VISUAL_DEBUGGING && UNITY_EDITOR)
                if (UnityEngine.Application.isPlaying) {
                    var poolObserver = new Entitas.Unity.VisualDebugging.PoolObserver(_uI);
                    UnityEngine.Object.DontDestroyOnLoad(poolObserver.entitiesContainer);
                }
                #endif
            }
